...
- **Behavior is missing** — the Goal needs an outcome no AC states → that is a **new Goal, not a finding to absorb**: report it as a proposed extension plan (own issue, own approval, parented per the PR Pattern rules) and leave this plan's AC set alone. Whether the extension runs at all is the user's call. Adding behavior inside a review is how a one-sentence request becomes a fourteen-AC plan: each addition arrives unreviewed and becomes the next round's defect surface.
...
**Two rounds, then landing checks.** The round number is the plan header's `Rounds:` field, never session memory — history prunes and sessions compact, but the count survives in the header, which is what makes this budget checkable at all. Entering with `Rounds: 0` or `1` is a full round: round 1 attacks; round 2 verifies the fixes landed and attacks what they changed. Entering with `Rounds:` ≥ 2 is a **landing check only** — did the fixes land, did they contradict anything — never a fresh hunt. A landing check that finds a genuine blocker reports it, and the fix gets one more landing check; **a blocking finding entered at `Rounds:` ≥ 3 is the second failed landing check and ends the lane** (entry at 2 is the first landing check — reaching 3 means it already failed once) — the plan goes back to design-feature for decomposition or rethink, never to round N+1. If a landing check wants to hunt, the plan is too large: decompose.

**Report only what changes the built artifact.** A finding is reportable when it changes what gets built, or when it lets a wrong implementation pass. Everything else — a rotted citation, a stale phrase, a prose contradiction between two sections that agree in substance — goes in one grouped `Nits:` line and is **never** a reason to withhold READY. Reporting everything makes every round look productive, which is what keeps justifying the next one.
...
On **every** verdict, READY or NEEDS CHANGES, the main agent increments the header's `Rounds:` field — rounds and landing checks alike; the count is the durable record the pruned history cannot be — appends one entry to `## Review History` at the end of the plan — a rejected round is the one a later reviewer gains most from — and **prunes it to the last two rounds**. Pre-approval the plan is an untracked locator (`worktree.md`), so a pruned round is *gone*, and that loss is deliberate: a round that mattered changed the AC set, and the change survives in the plan itself; what pruning discards is the argument, which is exactly the surface a later round re-litigates. A kept history that only ever grows becomes the largest section in the file — before this rule existed, one real plan's history reached ~700 lines across twelve rounds. Two rounds is what the budget above allows, so the section can never exceed it:
...
- [ ] **Budget and boundary:** the round number came from the header's `Rounds:` field, not memory, and the field was incremented with this verdict; entry at `Rounds:` ≤ 1 ran a full round, ≥ 2 stayed a landing check that opened no fresh hunt, and a blocker at entry ≥ 3 ended the lane; no finding of mine added, widened, or reinterpreted an AC — wrong-AC findings ended the round and routed to design, and missing behavior was proposed as an extension plan, never absorbed; every reported finding changes what gets built or admits a wrong implementation, and the rest are one grouped `Nits:` line; `## Review History` is pruned to the last two rounds. `Rounds:` now: __.
