...
3. **Too-broad test.** A goal is too broad when either fails:
   ...
   Neither test subsumes the other, and that order is why: **unity locates seams, count only signals size**. Unity alone drifts — at sufficient altitude every scope defends itself as one capability, which is how a thirty-outcome goal gets written and passes its own review. Count alone is semantically blind: it never fires on the LB-digest bundle above, and cutting on it alone severs a coherent capability at outcome #9.
...
- **A parent issue always exists at handoff — frame-goal guarantees it.** Arrived carrying one → *that* is the parent, untouched. Arrived with none → create it now (`gh issue create`: the requirement, the confirmed goal list, the dependencies between them) and it is the parent. This holds for a lone pass-through goal too: one issue is opened, not none.
- **Every goal links the parent by default** — goal 1 and each deferred goal alike. Splitting a requirement into goals is not a reason to split its issue: the single issue is what keeps a multi-goal framing legible afterwards, and each goal's plan links it as a parent (`design-feature.md` `## Issue`). Record the deferred goals in it as a GitHub task list — `- [ ] <goal sentence> — depends on <n>` — so each survives the session in a form the later phases can read; `create-pr` ticks an entry when that goal's PR is created, and an unticked entry is what stops an earlier sibling from closing the parent. Then hand the design skill that number so its header starts at `Issue: #<n>` and it links instead of opening a duplicate.
