...
- **AC budget: 8, and ≤3 clauses per AC** — this bullet is the single source for both numbers. The **clause surface, not the AC count**, is what both caps protect: the approval pause is one human judging the whole contract in one sitting, and review's two rounds can attack ≤24 clauses clause by clause. The cap sits at 8 because **a budget is an attractor, not merely a limit** — plans grow to fill whatever it allows, so a cap of 12 would ship surfaces of up to 36 clauses, past what two rounds verify. The price is deliberate: a goal that is genuinely one capability under `frame-goal`'s unity test can still exceed 8 — layered CRUD goals routinely land at 11–12 — and is then cut into slices thinner than any seam unity found, which `frame-goal` marks `thin` for the user. That buys verifiable review; it is not a claim that goals are naturally this size. Do **not** expect the ≤10-step cap to fire alongside it: steps are layer-shaped and near-constant for a given architecture while ACs scale with behavioral surface, so in layered work this cap fires alone and the step cap stays slack. Over budget → the Goal is too broad for one plan: STOP and return the requirement through `frame-goal` (single source for the too-broad test and split boundaries), which yields issue-backed goals, dependency-ordered via PR `Parent`, each with its own plan and approval. The clause cap keeps the AC cap honest: fewer, fatter ACs grow the same surface while evading the count.
...
**Every plan links exactly one issue, but supplying that issue is optional.** Creating one is the fallback, not the default: where the requirement, the user, or `frame-goal` already named an issue — including a **parent issue** several sibling plans share — link it. Linking happens before handoff, not at the execution gate, so the omission surfaces while the plan is still being written rather than after approval.

1. **Resolve the issue** — three cases:
   - **None named** → build a body from `## Goal`, `## Requirement`, the expected outcome, and `## Scope` (enough for someone who never reads the plan), `gh issue create`, and record the number. This is a fallback for a plan that reached design without `frame-goal`; normally frame-goal has already handed over a parent.
   - **A dedicated issue named** — a per-goal issue opened at the user's explicit request, or the same issue returning on a NEEDS CHANGES round → do **not** open a second one: fetch it, confirm it is open, and update its body where the Goal, Requirement, or Scope changed.
   - **A parent issue named** — the requirement issue `frame-goal` guarantees → link it, and how much you may write depends on how many goals it tracks. Tracking **several** → **leave the body alone**: rewriting a parent to one child's Goal and Scope destroys the shared record, so put anything this plan needs to say in a comment, and expect the PR to reference rather than close it (`create-pr.md` `## Publish`). Tracking exactly **one** — a lone pass-through framing — → it is effectively dedicated: enrich the body with this plan's Requirement, expected outcome, and Scope as in the dedicated case above.
2. **Record it.** The header reads exactly `Issue: #<n>` — `gate-check` matches `#<digits>` and nothing else, so a suffix such as `#12 (parent)` blocks the plan. When the issue is a shared parent, say so in `## Context` in as many words — `Issue #12 is a shared parent (goals 1–3)` — since that line is what review and create-pr read rather than re-deriving the relationship.

A named issue that is closed, or that belongs to different work → STOP and ask; never silently repoint the header. Credentials are PROCESS `Git credentials`; this skill adds nothing to them.
