...
Referenced by PROCESS gate #5 (thresholds, bands, and the stricter-only rule live there) and read by execute-feature/fix-bug at first scoring. PROCESS owns the gates; this file owns only measurement — so the measurement rules can't be edited to bypass a gate. Mechanics below; each names its **fallback** for when the stack can't measure what it asks.
...
**When this section applies:** a ❌, or a ⚠️ you have decided to close. A ⚠️ you log and carry stops at step 1 — name the behavior in the Coverage Gap entry and continue (PROCESS #5). Logging an uncovered behavior is not Discovered Scope and is not a STOP; only an attempt to *close* it can surface PROCESS #6 work.
...
⚠️ payments/refund.py — 84% branch (patch, diff-cover vs <base>)
   ...
   Carried, not closed: logging this is not Discovered Scope (PROCESS #5).
```
...
**Touched-line (patch) coverage** — PROCESS #5 gates lines *this change* touched, not the whole file. Where the run emits coverage XML, get patch granularity with `diff-cover coverage.xml --compare-branch=<base>`. Fallback (no XML/diff-cover): score the whole changed-file % (never repo-global). **That fallback dilutes most where the file is largest and best covered** — when scoring a whole file, read the uncovered-line list against the diff and gate touched lines by inspection. A ✅ no changed line earned is a false pass.
