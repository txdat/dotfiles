...
These override all other tendencies. Violating one → STOP and self-correct. Numbering is stable: #3 → CODING `No fake implementations`, #8 → CODING `Evidence, not memory`. Gaps are intentional — renumbering would break external references.
...
6. **Coverage gates are numeric — the number is a floor, not a score.** Coverage proves exercise, not correctness. Can't assert meaningfully → log a Coverage Gap; never write a test to raise the number (CODING `Report, don't decide`; smells: coverage.md `Quality bar`).
   ...
   **Carry, don't close.** A ⚠️ is logged and carried — never Discovered Scope, never a STOP. Closing a gap goes through behavior, never lines; a missing TC can surface as PROCESS #7 work only when closing a ❌ or chosen ⚠️.
