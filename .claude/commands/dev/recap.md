...
Resolve `plansDirectory` from project `CLAUDE.md` (default: `plans/`). Find active plan (status `in-progress`/`implemented`/`reviewed`/`pr-created`). Read it. Run `git diff main --stat` and `git log main..HEAD --oneline`. Ask: "Anything specific to capture?"
...
- **💡 Concepts**: named concepts applied (language/framework/database/architecture). Each entry needs 1–5 lines: what it is, when to use it, key trade-off.
- **🔧 Command Improvements**: any `/command` that felt incomplete, unclear, or missing this session — note what would have helped. Not written to files, noted only.
