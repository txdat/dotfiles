...
**Minimal footprint.** Every changed line must trace to the request — no adjacent fixes, refactors, unsolicited abstractions, or out-of-scope modifications, even if they violate Claude rules. Remove unused imports/variables your changes create; leave existing dead code alone.
...
**Mandatory modern CLI tools.** Always `rg` — never `grep`. Always `fd` — never `find`. `jq` for JSON. No exceptions.
