...
awful.layout.layouts = {
    ...
    -- awful.layout.suit.fair, -- fair.vertical
...
mytextclock = wibox.widget.textclock("  %a %m/%d %I:%M %p ")
...
local my_taglist = function(s)
    ...
    local unfocus_icon = " "
    local unfocus_color = "#727169"

    local empty_icon = " "
    local empty_color = "#727169"

    local focus_icon = " "
    local focus_color = "#ecf0c1"
...
local storage_home = awful.widget.watch(scripts_dir .. "disk-usage.sh", 60, function(self, stdout)
    st_home = stdout
...
awful.widget.watch(scripts_dir .. "get-cpu.sh", 2, function(self, stdout)
...
local br, br_signal = awful.widget.watch(scripts_dir .. "brightness-bar.sh", 60, function(self, stdout)
...
local vo, vo_signal = awful.widget.watch(scripts_dir .. "volume-bar.sh", 60, function(self, stdout)
    local vol = stdout
...
awful.screen.connect_for_each_screen(function(s)
    ...
    -- Create the wibox
    s.mywibox = awful.wibar({ position = "top", border_width = 0, border_color = "#00000000", height = 50,
...
root.buttons(gears.table.join(
    awful.button({}, 3, function() mymainmenu:toggle() end), -- right mouse
...
globalkeys = gears.table.join(
    ...
    awful.key({ modkey, "Shift" }, "s",
        function() awful.spawn("scrot -s -q 100 " .. os.getenv("HOME") .. "/Pictures/screenshots_%Y-%m-%d_$wx$h.png") end
        ,
    ...
    awful.key({}, "XF86MonBrightnessUp",
        function()
            awful.spawn(scripts_dir .. "brightness-up.sh")
    ...
    awful.key({}, "XF86MonBrightnessDown",
        function()
            awful.spawn(scripts_dir .. "brightness-down.sh")
            br_signal:emit_signal("timeout")
            -- local brightness = scripts_dir .. "brightness-bar.sh"
            -- awful.spawn.easy_async(brightness, function(stdout)
            --     naughty.notify {
